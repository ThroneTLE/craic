...
def top_view_shot(image_msg):
    ...
    if im_flag == 1:
        ...
        cv2.imwrite('/home/abot/throne_craic/src/abot_vlm/temp/vl_now.jpg', img_bgr)
        # 将im_flag重置为255
...
def doubao_vision_api(PROMPT='图片中有一个计算式，请计算一下结果并输出，例如：图中内容为1+1=，你输出为2。图中的内容为2+2=，你输出4。注意，你只输出结果，比如数字2,即最后的输出一定是一个数字，除了数字一定不要展示其他内容,我只要输出的数字格式为单个字符 例如8，在终端输出的格式为“结果：最终的数字”', img_path='/home/abot/throne_craic/src/abot_vlm/temp/vl_now.jpg'):
    '''
